...
def process(view, opts, modelfile, trainfile, testfile, featurefile, outdir, evaluator):
    ...
    if remaining == 0:
        ...
        logging.info(
            'concatenating {0} partial output files...'.format(len(done)))
        paths = [os.path.join(
            recsdir, 'recs.{0}-{1}.tsv'.format(start, end)) for start, end in done]
        cmd = ['cat'] + paths
        ...
        logging.error(
            'FAILED: {0}/{1} tasks did not complete successfully'.format(remaining, len(tasks)))
...
def create_tasks(modelfile,
    ...
    for start in xrange(0, num_users, users_per_task):
        ...
        tasks.append((modelfile, input_format, trainfile, test_input_format, testfile,
                      item_feature_format, featurefile, outdir, start, end, evaluator, generate))
    logging.info(
        'created {0} tasks, {1} users per task'.format(len(tasks), users_per_task))
    return tasks
...
def estimate_users_per_task(mb_per_task, input_format, trainfile, modelfile):
    ...
    if isinstance(model, MatrixFactorizationRecommender):
        ...
        if mb_per_task > required_mb_per_task:
            ...
            users_per_task = (
                (mb_per_task - required_mb_per_task) * ONE_MB) / (num_items * 16)
        ...
        if mb_per_task > required_mb_per_task:
            # estimate additional usage from avg items per user and sims per
            # item
            items_per_user = 1 if (nnz / num_users) == 0 else nnz / num_users

            sims_per_item = 1 if model.similarity_matrix.nnz / num_items == 0 else model.similarity_matrix.nnz / num_items
...
def main():
    ...
    parser.add_option(
        '--modeldir', dest='modeldir', help='directory containing trained models')
    parser.add_option(
        '--outdir', dest='outdir', help='directory for output files')
    ...
    parser.add_option('--overwrite', dest='overwrite', action='store_true',
                      default=False, help='overwrite existing files in outdir (default: %default)')
    parser.add_option('--packer', dest='packer', default='json',
                      help='packer for IPython.parallel (default: %default)')
    ...
    if opts.mb_per_task is None:
        ...
        # don't take *all* the memory
        opts.mb_per_task = psutil.virtual_memory().available / ONE_MB / \
            (num_engines + 1)
    ...
    if opts.add_module_paths:
        ...
        for path in opts.add_module_paths.split(','):
            logging.info(
                'adding {0} to pythonpath on all engines'.format(path))
    ...
    for trainfile in trainfiles:
        ...
        description, metrics = process(
            view, opts, modelfile, trainfile, testfile, opts.item_features, opts.outdir, evaluator)
    ...
    if len(descriptions) > 1:
        logging.warn(
            'You are aggregating metrics from different models! {}'.format(description))
