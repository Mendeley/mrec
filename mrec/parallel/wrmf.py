...
class WRMFRunner(object):
    def run(self, view, model, input_format, trainfile, num_engines, workdir, modelfile):
        ...
        for it in range(model.num_iters):
            ...
            tasks = self.create_tasks(num_users, num_engines, model, input_format, trainfile, workdir, 'U',
                                      get_user_indices, get_item_factor_files, init_item_factors)
            self.run_tasks(view, tasks)
            tasks = self.create_tasks(num_items, num_engines, model, input_format, trainfile, workdir, 'V',
                                      get_item_indices, get_user_factor_files,
    ...
    def create_tasks(self, num_factors, num_engines, model, input_format, trainfile, workdir, factor_type, get_indices,
                     get_fixed_factor_files, init_fixed_factors):
        ...
        for start in range(0, num_factors, factors_per_engine):
            ...
            tasks.append((model, input_format, trainfile, factor_type, get_indices, init_fixed_factors,
                          fixed_factor_files, start, end, workdir))
        return tasks
